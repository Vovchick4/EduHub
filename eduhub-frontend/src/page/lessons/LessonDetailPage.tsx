...
const LessonDetailPage = () => {
  ...
  if (!Number.isInteger(courseId) || !Number.isInteger(parsedLessonId))
    return <p>Некоректне посилання на урок.</p>
  ...
  const deleteLesson = () => {
    ...
    deleteMutation.mutate(
      { courseId, lessonId: parsedLessonId },
      { onSuccess: () => navigate(`/courses/${courseId}`) },
    )
  }
  const canEdit =
    profileQuery.data?.role === 'admin' || profileQuery.data?.email === courseQuery.data?.author
  return (
    <section className="lesson-detail-container">
      ...
      <article className="lesson-card">
        ...
        <p className="lesson-description">{lesson.content || 'Матеріал уроку ще не додано.'}</p>
        <div className="lesson-meta">
          <ul>
            <li>Створено: {new Date(lesson.created_at).toLocaleString('uk-UA')}</li>
            <li>Оновлено: {new Date(lesson.updated_at).toLocaleString('uk-UA')}</li>
          </ul>
        </div>
        {canEdit && (
          <div className="lesson-actions">
            <Link to={`/courses/${courseId}/lessons/${parsedLessonId}/update`} className="btn-card">
              ✏️ Редагувати
            </Link>
            <button
